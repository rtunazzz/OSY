...
class CounterClass {
   ...
   public:
    ...
    void firstShift() {
        ...
        cout << "G1: Thread:  counter = " << g_Cnt << endl;
    }
    ...
    void secondShift() {
        ...
        ul.lock();
        g_Cond1.notify_all();
        cout << "G2: Thread:  counter = " << g_Cnt << endl;
    }
};
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < 4; i++)
        workers.push_back((i < 2) ? thread(&CounterClass::firstShift, &counterObj) : thread(&CounterClass::secondShift, &counterObj));
