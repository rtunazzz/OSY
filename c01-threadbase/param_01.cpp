...
void threadFunc(int value, vector<int> values) {
    ...
    printf("Thread:   value = %d     values[1] = %d     values.data = %p\n", value, values[1], (void *)values.data());
...
int main(int argc, char *argv[]) {
    ...
    printf("Main:     Start\n");
    printf("Main:     value = %d     values[1] = %d     values.data = %p\n", value, values[1], (void *)values.data());

    // Create thread and wait for completion
    printf("Main:     Creating thread\n");
    thread(threadFunc, value, values).join();
    printf("Main:     value = %d     values[1] = %d     values.data = %p\n", value, values[1], (void *)values.data());
